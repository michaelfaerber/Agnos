package alu.linking.preprocessing.stemmer;
...
//		word = word.replaceAll("\\.", "\n");
		word = word.replaceAll("(?<!\\d)\\.(?!\\d)", " "); //ADDED
		word = word.replaceAll("(?<!\\d)\\,(?!\\d)", " ");
		word = word.replaceAll("[\\p{Punct}&&[^0-9]&&[^-]&&[^.]&&[^,]]", ""); //ADDED
	...
	public static String stemSentence(String text, boolean case_insensitive) {
		String[] tokens = text.split(" ");
		StringBuilder sb = new StringBuilder();
		
		for(String token: tokens) {
			sb.append(stem(token, case_insensitive) + " ");
		}
