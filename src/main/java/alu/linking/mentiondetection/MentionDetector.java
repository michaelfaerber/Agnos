...
public interface MentionDetector {
	 ...
	 * @return
	 */
	default List<Mention> removeLongerCollisions(List<Mention> mentions, final boolean removeStartOffset, final boolean removeEndOffset) {
		...
		for (int i = 0; i < copyMentions.size(); ++i) {
			...
			for (int j = i + 1; j < copyMentions.size(); ++j) {
				...
				if (i != j && sameSF && !outerMention.getOriginalMention().equals(innerMention.getOriginalMention())) {
					...
					if (false //
							|| (removeStartOffset && sameStartOffset)
					|| (removeEndOffset && sameEndOffset)
					) {
